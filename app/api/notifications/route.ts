...
export async function GET() {
  ...
  if (user.role === "student") {
    ...
    const { data: newApps } = await sb
      .from("applications")
      .select("id, procedure_id, status, submitted_at, procedures(name), student_profiles!applications_user_id_fkey(full_name)")
      .gte("submitted_at", since)
      .in("status", ["submitted", "more_info_requested"])
      .order("submitted_at", { ascending: false })
      .limit(15);
    for (const a of (newApps ?? []) as unknown as Array<{
      id: string; procedure_id: string; status: string; submitted_at: string;
      ...
      events.push({
        type: "new_submission",
        title: `New submission · ${a.student_profiles?.full_name ?? "Student"}`,
    ...
    const { data: decisions } = await sb
      ...
      .select("id, application_id, decision, decided_at, applications!inner(procedure_id, procedures(name), student_profiles!applications_user_id_fkey(full_name))")
    ...
    for (const d of (decisions ?? []) as unknown as Array<{
      ...
      applications: { procedure_id: string; procedures?: { name: string }; student_profiles?: { full_name: string } };
    }>) {
      events.push({
        type: "decision",
        ...
        subtitle: `${d.applications?.student_profiles?.full_name ?? ""} · ${d.applications?.procedures?.name ?? d.applications?.procedure_id ?? ""}`,
