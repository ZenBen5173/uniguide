...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data: app } = await sb
    .from("applications")
    .select("id, user_id, procedure_id, status, progress_current_step, progress_estimated_total, student_summary, ai_recommendation, ai_confidence, created_at, submitted_at, decided_at, procedures(name, description)")
    .eq("id", id)
    .single();
